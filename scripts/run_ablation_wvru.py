#!/usr/bin/env python3
"""Cross-binarization validation of the canton-level ablation (referee-flagged
gap: run_ablation.py's headline +1.2pp canton result uses the 2026 coalition
split; the distrito-level ablation was already cross-checked against the
winner-vs-runner-up (wvru) binarization in run_distrito_ablation_wvru.py, but
the canton side never was, leaving open whether the canton-level near-null
result is itself an artifact of the specific 3-party coalition construction.

Same geography-only vs. geography+margin ablation, same budget as
run_ablation.py (8 pooled seeds, 500+500 sweeps, 24 temperatures), same
canton adjacency network -- only the binarization changes: 2026's top two
vote-getters head to head, matching run_historical_comparison.py's 2026 row
and run_distrito_ablation_wvru.py exactly, one administrative level up.
"""
...
def main():
    ...
    for label, r in (("A: geography only", run_a), ("B: geography+h", run_b)):
        ...
        print(f"{label:20}{r['N']:>4}{baseline:>10.1%}{best_str:>13}"
              ...
              f"{TEMPERATURES[peak_idx]:>7.2f}{mc['median_exact_pvalue']:>11.5f}{sig_str:>7}")

    gap = run_b["accuracy"].max() - run_a["accuracy"].max()
    print(f"\nMarginal contribution of predisposition (h) over pure geography, at "
          f"canton granularity, winner-vs-runner-up binarization ({winner} vs. "
          f"{runner_up}): {gap:+.1%}")
    print("Compare to coalition-split canton result (run_ablation.py): +1.2pp.")

    np.savez(RESULTS_DIR / "ablation_wvru.npz",
    ...
    print(f"Raw results written to {RESULTS_DIR / 'ablation_wvru.npz'}")
