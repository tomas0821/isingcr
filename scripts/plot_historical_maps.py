...
sys.path.insert(0, str(Path(__file__).resolve().parent.parent / Path("src")))
...
DATA_RAW = Path(__file__).resolve().parent.parent / Path("data/raw")
OUT = Path(__file__).resolve().parent.parent / Path("manuscript/figures/historical_maps.png")
...
ELECTIONS = [
    {
        ...
        # 2018 is the one contest where the winner (PAC) carried the capital
        # region and the runner-up the periphery; swap the colours so that
        # red marks the periphery side in all three panels.
        "swap_colors": True,
    },
    {
        "label": "2022 (runoff)",
...
ACCENTS = {"ACCION CIUDADANA": "Acción Ciudadana", "RESTAURACION NACIONAL": "Restauración Nacional",
           "PROGRESO SOCIAL DEMOCRATICO": "Progreso Social Democrático", "LIBERACION NACIONAL": "Liberación Nacional",
           "PUEBLO SOBERANO": "Pueblo Soberano"}


def _title_case(party):
    return ACCENTS.get(party.strip().upper(), " ".join(w.capitalize() for w in party.split()))
...
for ax, election in zip(axes, ELECTIONS):
    ...
    colors = [RUNNERUP_COLOR, WINNER_COLOR]
    if election.get("swap_colors"):
        colors = colors[::-1]
