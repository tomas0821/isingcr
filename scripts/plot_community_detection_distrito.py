#!/usr/bin/env python3
"""Distrito-level companion to plot_community_detection.py: Louvain
community detection on the distrito-level coupling network J_ij alone (no
vote/political data), compared against the 7 official provinces and
GAM/periphery membership. A one-level-down robustness check on whether the
canton-level result (communities cross provinces, partially track GAM)
holds at finer resolution too.
"""
import sys
...
def canton_of(distrito_code):
    parts = distrito_code.split("|")
...
gdf = gdf.loc[[n for n in gdf.index if n in comm_of]]
...
gam_labels = np.array([canton_of(n) in GAM_CANTONS for n in nodes])
...
ax = axes[0]
gdf.plot(ax=ax, column="community", cmap=cmap, categorical=True, edgecolor="black", linewidth=0.1)
...
ax = axes[1]
gam_color = gdf.index.map(lambda c: "tab:red" if canton_of(c) in GAM_CANTONS else "#dddddd")
gdf.plot(ax=ax, color=gam_color, edgecolor="black", linewidth=0.1)
for i, comm in enumerate(comms):
    sub = gdf.loc[[n for n in comm if n in gdf.index]]
    if len(sub) == 0:
        continue
    sub_dissolved = sub.dissolve()
    sub_dissolved.boundary.plot(ax=ax, edgecolor=cmap(i % 20), linewidth=1.3)
