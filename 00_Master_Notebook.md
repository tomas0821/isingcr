...
boundary). See `README.md` for the full writeup and `docs/NOTES.md` for the gotchas list
...
`pooled_temperature_scan`'s docstring and `docs/NOTES.md` gotcha #7:
...
was an overstatement of what the data supports. README/docs/NOTES.md updated with this
...
metastable-freezing artifact (docs/NOTES.md gotcha #7 — the classic failure this project has
...
between-chain-disagreement artifact (docs/NOTES.md gotcha #7) -- a real thermodynamic
...
### Run: lambda_soc scan of the political-continuity field (2022 runoff margin), 2026, N=488 — 2026-09-05

**Scripts**: `scripts/run_prior_lambda_scan.py` + `scripts/submit_prior_lambda_scan.slurm` (UCR `shared`, 32 cores,
SLURM job 127370, 9 array tasks, 29-32 min each); analysis `scripts/prior_lambda_scan_analysis.py`; paired tests
`scripts/run_prior_paired_test_at_lambda.py` (local, 99,999 draws). **Data**: `data/processed/prior_lambda_scan_2026_lam{0..8}.npz`,
`prior_paired_test_lam6_2026.npz`, `prior_paired_test_lam16_2026.npz`. Figure `manuscript/figures/gam_lambda_scan.png`
regenerated with the political curve (`plot_gam_lambda_scan.py`).

Motivation: the manuscript ranked GAM above the political field at unit weight only; the political field has sigma=0.167
vs +/-1 for GAM (a sixth of the nominal strength) and, on the coalition split actually scored, the same sign-agreement
ceiling (80.6% vs 80.9%). Same budget/seeds/grid as the GAM scan (16 seeds, 20000+20000, 32 T in [0.05,5]).

| lambda | best alignment | T* | McNemar p | E_J | E_h | field share | follows field |
|---|---|---|---|---|---|---|---|
| 0.5 | 68.34 +/- 5.13 | 2.605 | 0.096 | 1102 | 14 | 0.012 | 64.3% |
| 1 | 69.79 +/- 1.82 | 2.924 | 0.27 | 1088 | 33 | 0.029 | 66.2% |
| 2 | 74.08 +/- 3.12 | 2.126 | 3.1e-5 | 1182 | 74 | 0.059 | 69.1% |
| 3 | 77.19 +/- 3.73 | 0.848 | 8.2e-7 | 1251 | 137 | 0.099 | 74.3% |
| 4 | 78.69 +/- 2.92 | 0.848 | 7.6e-9 | 1254 | 191 | 0.132 | 75.5% |
| 6 | 81.40 +/- 1.25 | 0.848 | 1.2e-12 | 1248 | 309 | 0.199 | 77.9% |
| 8 | 81.72 +/- 1.90 | 0.529 | 1.9e-11 | 1211 | 434 | 0.264 | 80.0% |
| 12 | 85.07 +/- 1.99 | 0.369 | 4.9e-14 | 1184 | 697 | 0.371 | 83.7% |
| 16 | 86.05 +/- 1.23 | 0.529 | 3.5e-16 | 1193 | 949 | 0.443 | 85.9% |
| 24 | 84.55 +/- 1.19 | 0.689 | 1.2e-13 | 1143 | 1481 | 0.564 | 88.6% |
| 32 | 83.77 +/- 0.80 | 0.689 | 2.2e-12 | 1096 | 2032 | 0.650 | 90.7% |
| 48 | 82.30 +/- 0.84 | 0.529 | 1.2e-9 | 1018 | 3156 | 0.756 | 93.1% |
| 64 | 81.93 +/- 0.65 | 0.369 | 9.6e-10 | 984 | 4252 | 0.812 | 94.2% |

lambda=1 reproduces the headline unit-weight run (69.79%). Rows 24-64 are a second submission (SLURM 127389,
`submit_prior_lambda_scan_ext.slurm`, array 9-12) added because the first nine points were still rising at 16.
**Peak at lambda*=16 (86.05%)**, then a decline toward the 80.6% asymptote; sigma-scaled optimum 16*0.167 ~ 2.7 mean
couplings vs GAM's 1.5. Alignment exceeds the 80.6% sign-agreement ceiling because the continuous field leaves many near-zero units for the
coupling to decide (follows-field only 85.9% at lambda=16), so the ceiling is an asymptote here, not a bound.

Paired vs geography-only (99,999 draws, canton blocks): lambda=6 (sigma-matched to GAM) median p=0.00016, 15/16,
Bonferroni x32 = 0.005. lambda=16 (T=0.529): median p=0.00001 (6/16 seed pairs at zero exceedances even with 99,999 draws), 16/16 significant, Bonferroni x32 < 0.001.
Head-to-head vs GAM (saved best-T spins, same seeds): pol lambda=6 vs GAM lambda=1: +0.72 pts, p=0.82, 0/16 (pol ahead
10/16 seeds) -> tie. pol lambda=16 vs GAM lambda*=1.5: +4.30 pts, pol ahead in 16/16 seeds, but block-permutation
p=0.35, 0/16 (the two fields disagree in few cantons, so the block test has little power).

**Conclusion**: the field ranking is weight-dependent. GAM is strongest at unit weight, with its optimum at the smallest
weight; weight-optimized, the prior vote matches GAM at matched magnitude and exceeds it at its own optimum (lambda=16), without the two being statistically distinguishable at any matched point. Manuscript abstract, intro,
Sec. 4.5/4.6, Discussion, Conclusion, highlights and SM S9.1 (new Table S) updated accordingly.

### Run: residualized GAM field (development + population regressed out), 2026, N=488 — 2026-09-07

**Scripts**: `scripts/run_gam_residual_field.py` + `scripts/submit_gam_residual_field.slurm` (UCR shared, SLURM 127660,
3 tasks ~30 min each), `scripts/registered_voters.py` (electores_inscritos per distrito from the consolidado ZIP),
`scripts/run_resid_paired_test.py`. **Data**: `data/processed/gam_residual_field_2026_lam{0,1,2}.npz`,
`resid_paired_test_lam1.5_2026.npz`, `resid_paired_test_lam2_2026.npz`.

Field: OLS of the +/-1 GAM field on z(IDS) and z(log registered voters): beta = [-0.238, 0.566, 0.101], R^2 = 0.40;
z-scored residual. r(resid, IDS) = r(resid, logpop) = 0 by construction; r(resid, GAM) = 0.775; r(resid, outcome) =
-0.318 (GAM -0.589, IDS -0.554, logpop -0.195); sign-agreement ceiling 69.1% (GAM 80.9, IDS 69.3); sign(resid) ==
sign(GAM) on 87.7% of nodes.

| lambda | best alignment | T* | McNemar p |
|---|---|---|---|
| 1.0 | 75.87 +/- 5.12 | 1.806 | 3.1e-5 |
| 1.5 | 78.68 +/- 2.18 | 1.806 | 8.9e-6 |
| 2.0 | 79.42 +/- 0.69 | 1.966 | 1.5e-5 |

Paired vs geography-only (99,999 draws, canton blocks): lambda=1.5 median p=0.0054, 12/16, x32 = 0.17 (fails the grid
correction). Head-to-head vs raw GAM lambda=1: GAM ahead by 2.25 pts in 16/16 seeds, median p=0.048 (8/16).
lambda=2 paired: median p=0.00233, 14/16, x32 = 0.074 (still fails the grid correction at 0.05); vs raw GAM: GAM ahead by 1.84 pts in 16/16 seeds, median p=0.113 (6/16).

**Conclusion**: the part of GAM that development and population cannot explain keeps +8 to +12 of GAM's +13.4/+13.8
points (more than the IDS field's +6.8 at its optimum) but with weaker significance; GAM is not reducible to
development + population, though both contribute. Written into main Sec. 4.5, Table 2 (survives table), abstract,
Discussion, Conclusion, Limitations, SM S9 (new subsection + table).

### Analysis: regression baselines on the same covariates — 2026-09-07

`scripts/run_regression_baselines.py` (numpy/scipy logistic regression, L2 1e-3). 2026 distrito outcome, N=488,
baseline 67.01%. In-sample / leave-one-province-out: GAM only 80.94 / 80.94; GAM+IDS+logpop 84.02 / 81.56;
+lag(GAM) 84.02 / 80.33; autologistic (+lag of OBSERVED outcome) 89.96 / 89.34; lag(y) only 89.96 / 89.75.
The Ising GAM fit (81.07 / 81.47 at optimum, in-sample) is NOT more accurate than a plain logistic regression on the
same covariates; the manuscript now says so in the Introduction, Sec. 4.5 and SM (new table), and frames the Ising
model's value as the dynamical diagnostics.

### Referee round 7 fixes + lambda=16 paired test at 10^6 draws — 2026-09-07

`referee_report_2026-09-07.md` (16 verified majors). Data items: (1) `run_prior_paired_test_at_lambda.py --lam 16 --t 0.529
--n-perm 999999` -> `prior_paired_test_lam16_2026_perm999999.npz`: median p = 3.0e-6 (plain) / 4.0e-6 ((b+1)/(m+1)),
5/16 seed pairs still at zero exceedances (8/16 at 99,999), 16/16 significant; x32 = 9.6e-5/1.3e-4, x32x8 = 7.7e-4/1.0e-3,
x32x8x13 = 0.010/0.013 -> clears every correction under both conventions (Table 2 updated). (2) Autologistic
beta_GAM/beta_lag(y) = 0.073 (full model; -0.10 with GAM+lag only): the regression's nearest analog of lambda*, small
because observed neighbor labels absorb the map; reported in SM S9.3 and used to reword the "no regression analog"
claim as generative-vs-conditional. (3) Combination check corrected: adding IDS on top of lambda_pol=2 LOWERS
alignment (82.36->66.05 at soc=1.5 in 2026; 70.10->59.92 in 2022); the old "+0.0 points" meant best lambda_soc=0.
Text: abstract p=0.0013 and "no fitted field weight"; 2026 "single round" + 82%-coverage caveat; PPSD wording;
Monteverde/Puerto Jimenez wording; comparators Godoy-Lorite & Jones and Massoli engaged in the Intro; Biswas et al.
2021 (Physica A 566) and Bokanyi et al. 2018 (PLOS ONE) cited; Chavarria/Camacho attribution corrected; Galam 2025
wording corrected; 2030 statement relabeled a model-internal bound, fault-line distritos made the falsifiable item;
domain-wall "both years" corrected; single "central" claim; Fig. 4 on a log lambda axis; Fig. 3 accents.

### The official GAM boundary vs. the canton proxy — 2026-09-08

Boundary: MIVAH GeoExplora "GAM LIMITE 1" (Plan GAM 2013-2030 polygon, EPSG:5367, 1779.8 km^2 = the plan's own
figure; bbox matches Decreto 38334 art. 2), saved under `data/raw/gam/` (README + shapefile, force-added).
`data/processed/gam_distrito_fraction_2026.csv`: area fraction of each of the 488 network distritos inside the polygon
-> 147 fully inside, 28 split, 313 outside. Two pre-fixed readings: area majority (>=0.5, 162 distritos) and any
overlap (>0.01, 175); proxy has 186. Majority rule: proxy over-includes 24 (Aserri, Mora, Atenas, Desamparados,
Coronado, El Guarco fringe; `manuscript/gam_boundary_table.tex`), omits none. Sign ceilings 80.94 (proxy) / 81.56
(any) / 80.53 (majority).

Runs (`run_gam_true_field.py`, SLURM 128119, 32-point grid, 16 seeds, lambda 1 and 1.5): majority 79.24+/-0.59 /
79.88+/-0.13; any 80.65+/-1.21 / 81.43+/-0.37; proxy 81.07 / 81.47. Paired vs geography-only at unit weight, 10^6
draws (`run_true_gam_paired_test.py`): any p=0.0010 (14/16; x32=0.032), majority p=0.0012 (15/16; x32=0.038).
Head-to-head vs proxy (same seeds): any -0.10 pts, 8/16, p=0.52 (tie); majority -2.05, 1/16, p=0.23.
LOPO on the official boundary (`run_true_gam_spatial_cv.py`): Alajuela still fails (-16.4 majority p=1e-4;
-18.1 any p<1e-4), Heredia +12.8 (p=0.03) both, San Jose +5.7/+9.8, majority also loses Cartago (-15.4, p=0.15).

**Key finding**: 13 of Alajuela Central's 14 distritos lie inside the official polygon (12 fully), yet all 14 voted
periphery -> the earlier "proxy-fix" relabeling (variant A, 84.7%) was an outcome-selected fix for a genuine
exception, not a boundary correction. The 24 over-included fringe distritos vote 46% leading vs 30% inside the
polygon, so the strict boundary removes contrast and scores lower; the inclusive boundary ties the proxy. Proxy
stays as headline (pre-specified, best); official boundary reported as check + finding. Written into main Sec. 2.4,
